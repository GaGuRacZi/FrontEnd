...
export function WalkRecordScreen() {
	...
	const [isEditing, setIsEditing] = useState(!params.recordId);
	const [detailLoading, setDetailLoading] = useState(Boolean(params.recordId && !cachedRecord));
	...
	useEffect(() => {
		if (!params.recordId || isEditing) return;
		let active = true;
		void getWalkRecord(params.recordId)
			.then((record) => {
				if (!active) return;
				setRemoteRecord(record);
				setIntensity(record.intensity);
				setUrination(record.excrement.urination);
				setDefecation(record.excrement.defecation);
				setSpecialNote(record.excrement.specialNote);
				setWeatherText(record.weatherText ?? null);
				setTemperatureText(record.temperatureText?.replace(/[^0-9.-]/g, '') ?? '');
				setRecordedAt(parseParamDate(record.date));
				const start = parseTimeStr(record.startTime);
				const end = parseTimeStr(record.endTime);
	...
	}, [cachedRecord, detailRequest, isEditing, params.recordId]);
	...
	const handleSaveRecord = async () => {
		...
		try {
			...
			Alert.alert('저장할 수 없어요', getHealthRequestErrorMessage(error, '산책 기록을 저장하지 못했어요. 잠시 후 다시 시도해주세요.'));
		} finally {
	...
	if (detailLoading) {
		return <AppScreen><TopHeader leftAccessibilityLabel="뒤로가기" leftIcon="chevron-back" onLeftPress={() => router.back()} title="산책 기록하기" /><LoadingView label="산책 기록을 불러오고 있어요." /></AppScreen>;
	}

	if (detailLoadError && !existingRecord) {
		return <AppScreen><TopHeader leftAccessibilityLabel="뒤로가기" leftIcon="chevron-back" onLeftPress={() => router.back()} title="산책 기록하기" /><EmptyState actionLabel="다시 시도" onActionPress={() => { setDetailLoading(true); setDetailRequest((current) => current + 1); }} title="산책 기록을 불러오지 못했어요" /></AppScreen>;
	}
