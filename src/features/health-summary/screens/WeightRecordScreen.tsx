...
import { AppButton, AppIcon, DatePickerSheet, EmptyState, LoadingView, TimePickerSheet } from '@/src/components/common';
...
import { getHealthRequestErrorMessage, getWeightRecord } from '../services/healthSummaryApi';
...
export function WeightRecordScreen() {
	...
	const cachedRecord = recordId ? weightRecords.find(({ id }) => id === recordId) : undefined;
	const [remoteRecord, setRemoteRecord] = useState<WeightRecord>();
	...
	useEffect(() => {
		if (!recordId || !selectedPet || isEditing) return;
		let active = true;
		void getWeightRecord(selectedPet.id, recordId)
			.then((record) => {
				if (!active) return;
				setRemoteRecord(record);
				setRecordedAt(parseRecordedAt(record.date, record.time));
				setWeight(String(record.weight));
				setBodyCondition(record.bodyCondition);
				setAppetite(record.appetite);
				setMemo(record.memo ?? '');
				setSelectedImageUri(record.photoUri ?? null);
	...
	}, [cachedRecord, detailRequest, isEditing, recordId, selectedPet]);
	...
	const handleSaveRecord = async () => {
		...
		try {
			...
			Alert.alert('저장할 수 없어요', getHealthRequestErrorMessage(error, '체중 기록을 저장하지 못했어요. 잠시 후 다시 시도해주세요.'));
		} finally {
	...
	if (!petsReady) {
		return <AppScreen><TopHeader leftAccessibilityLabel="뒤로가기" leftIcon="chevron-back" onLeftPress={() => router.back()} title="체중 기록하기" /><LoadingView label="반려동물 정보를 불러오고 있어요." /></AppScreen>;
	}

	if (!selectedPet) {
		return <AppScreen><TopHeader leftAccessibilityLabel="뒤로가기" leftIcon="chevron-back" onLeftPress={() => router.back()} title="체중 기록하기" /><EmptyState actionLabel={petLoadError ? '다시 시도' : '반려동물 등록'} onActionPress={petLoadError ? reloadPets : () => router.push('/pet/add' as Href)} title={petLoadError ? '반려동물 정보를 불러오지 못했어요' : '등록된 반려동물이 없어요'} /></AppScreen>;
	}

	if (detailLoading) {
		return <AppScreen><TopHeader leftAccessibilityLabel="뒤로가기" leftIcon="chevron-back" onLeftPress={() => router.back()} title="체중 기록하기" /><LoadingView label="체중 기록을 불러오고 있어요." /></AppScreen>;
	}

	if (detailLoadError && !existingRecord) {
		return <AppScreen><TopHeader leftAccessibilityLabel="뒤로가기" leftIcon="chevron-back" onLeftPress={() => router.back()} title="체중 기록하기" /><EmptyState actionLabel="다시 시도" onActionPress={() => { setDetailLoading(true); setDetailRequest((current) => current + 1); }} title="체중 기록을 불러오지 못했어요" /></AppScreen>;
	}
