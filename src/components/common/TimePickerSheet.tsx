				...
				<Animated.View onStartShouldSetResponder={() => true} style={[styles.sheet, { transform: [{ translateY: sheetY }] }]}>
					...
					<View style={styles.timePickerWrap}>
						<View style={styles.timePickerCol}>
							<Pressable onPress={() => setHour(h => (h + 1) % 24)} style={styles.timeArrowBtn}>
							...
							<Pressable onPress={() => setHour(h => (h - 1 + 24) % 24)} style={styles.timeArrowBtn}>
						...
						<View style={styles.timePickerCol}>
							<Pressable onPress={() => setMinute(m => (m + 5) % 60)} style={styles.timeArrowBtn}>
								<AppIcon color={COLORS.primary} name="chevron-up" size={28} />
							</Pressable>
							<View style={styles.timeValueBox}>
								<Text style={styles.timePickerValue}>{String(minute).padStart(2, '0')}</Text>
							</View>
							<Pressable onPress={() => setMinute(m => (m - 5 + 60) % 60)} style={styles.timeArrowBtn}>
								<AppIcon color={COLORS.primary} name="chevron-down" size={28} />
							</Pressable>
							<Text style={styles.timePickerUnit}>분</Text>
						</View>
					</View>
					<Pressable onPress={handleConfirm} style={styles.submitBtn}>
